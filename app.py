...
    # ------------------------------------------------------------------
    # Cette règle ne s'applique que si :
    # 1. Le mode de relève est 'Manuelle'.
    # 2. La marque est 'ITRON', 'SAPPEL (H)' ou 'SAPPEL (C)'.
    # 3. La norme FP2E est respectée.
    
...
    condition_specifique_manuelle = (is_mode_manuelle) & (is_sappel | is_itron) & (is_fp2e_compliant)
    
    # On applique la règle uniquement si la condition ci-dessus est VRAIE
    df_with_anomalies.loc[condition_specifique_manuelle & (~df_with_anomalies['Numéro de compteur'].str.lower().str.startswith(('i', 'd'), na=False)), 'Anomalie'] += 'Manuelle: Compteur doit commencer par "I" ou "D" / '
...
                # Mise à jour de la clé ici pour refléter le changement de logique
